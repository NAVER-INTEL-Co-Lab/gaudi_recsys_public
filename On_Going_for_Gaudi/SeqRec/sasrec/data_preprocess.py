...
def preprocess(fname):
    ...
    # dataset = load_dataset("McAuley-Lab/Amazon-Reviews-2023", f"5core_timestamp_{fname}", trust_remote_code=True)
    ...
    for u in tqdm(User.keys()):
        for i in range(len(User[u])-3):
            transition_dictnary[f'{str(User[u][i])}_{str(User[u][i+1])}'] +=1
            raw_dict[User[u][i]] = max(raw_dict[User[u][i]], transition_dictnary[f'{str(User[u][i])}_{str(User[u][i+1])}'])
        
    transition_score = defaultdict(int)
    for u in tqdm(User.keys()):
        for i in range(len(User[u])-3):
            transition_score[u] += (transition_dictnary[f'{str(User[u][i])}_{str(User[u][i+1])}']/raw_dict[User[u][i]])
        if len(User[u]) -3 >0:
            transition_score[u] /= (len(User[u])-3)
    # factor = 1.0/sum(transition_score.values())
    # for key in transition_score.keys():
    #     transition_score[key] = transition_score[key]*factor
        
    scores = list(transition_score.values())
    s = np.quantile(scores, 0.75)
    print(s)
    use_key = []
    min_lenght = 9999
    for key, value in tqdm(transition_score.items()):
        if value > s:
            if len(User[key]) < 20:
                use_key.append(key)
                min_lenght = min(min_lenght, len(User[key]))
    print(min_lenght)
    
    sample_size = int(len(use_key))
    use_key = random.sample(use_key, sample_size)
...
def preprocess_raw(fname):
    ...
    use_key = random.sample(list(User.keys()), int(sample_size*0.02))#Automotive 0.02 Electronics 0.01 Movies 0.02
